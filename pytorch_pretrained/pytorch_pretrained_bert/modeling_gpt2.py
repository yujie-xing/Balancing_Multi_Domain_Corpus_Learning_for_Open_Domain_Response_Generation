...
class GPT2LMHead(nn.Module):
    ...
    def set_embeddings_weights(self, model_embeddings_weights):
        ...
        self.decoder = nn.Linear(embed_shape[1], self.vocab_size, bias=False)
        self.decoder.weight.data = model_embeddings_weights.data[:self.vocab_size]  # Tied weights
...
class GPT2CLSHead(nn.Module):
    """ Classifier Head for the transformer """

    def __init__(self, config):
        super(GPT2CLSHead, self).__init__()
        self.n_embd = config.n_embd
        self.linear = nn.Linear(config.n_embd, config.n_cls, bias=False)
        self.tanh = nn.Tanh()
    ...
    def forward(self, hidden_states):
        ...
        # hidden_state (bsz, hidden_size)
        cls_logits = self.linear(hidden_states.sum(1))
        cls_logits = self.tanh(cls_logits)
        # (bsz, num_choices)
        return cls_logits
...
class GPT2MultipleChoiceHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.n_embd = config.n_embd
        self.linear = nn.Linear(config.n_embd, 1, bias=False)
...
class GPT2PreTrainedModel(nn.Module):
    ...
    def set_n_special(self, n_special, n_token):
        pass

    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path, n_special=None, n_token=None, n_cls=None, state_dict=None, cache_dir=None, from_tf=False, *inputs, **kwargs
    ):
        """
        ...
        # model.set_tied()
        model.set_n_special(n_special if n_special is not None else config.n_special, n_token if n_token is not None else config.n_token)
        return model
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.wte=nn.Embedding(config.vocab_size+config.n_special, config.n_embd)
        # self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.tte = nn.Embedding(config.n_token, config.n_embd)
    ...
    def set_n_special(self, n_special, n_token):
        ...
        if self.config.n_special == n_special:
            return
        # Update config
        self.config.n_special = n_special
        self.config.n_token = n_token
        ...
        self.wte.weight.data[self.config.vocab_size]=0
        ...
        self.tte=nn.Embedding(self.config.n_token, self.config.n_embd)
        self.init_weights(self.tte)
        self.tte.weight.data[0]=0


    def forward(self, input_ids, position_ids=None, token_type_ids=None, past=None, mask=None):
        if past is None:
            past_length = 0
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def set_n_special(self, n_special, n_token):
        ...
        """
        self.transformer.set_n_special(n_special, n_token)
        self.lm_head.set_embeddings_weights(self.transformer.wte.weight)

    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, past=None, mask=None):
        
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past, mask)
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.transformer = GPT2Model(config)
        self.lm_head = GPT2LMHead(self.transformer.wte.weight, config)
        self.cls_head = GPT2CLSHead(config)
        self.vocab_size = config.vocab_size
        self.apply(self.init_weights)
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, cls_id=None, cls_labels=None, past=None, mask=None, weight=None):
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past, mask)
        lm_logits = self.lm_head(hidden_states)
        if cls_id is not None:
            cls_logits = self.cls_head(hidden_states[:,:cls_id,:])
        losses = []
        if lm_labels is not None:
        ...
        if cls_labels is not None:
            ...
            losses.append(loss_fct(cls_logits, cls_labels))
        if losses:
            return losses
        try:
            return lm_logits, cls_logits, presents
        except NameError:
            return lm_logits, presents
...
#     def set_n_special(self, n_special):
#         """ Make sure we are sharing the embeddings
#         """
#         self.transformer.set_n_special(n_special)
#         self.lm_head.set_embeddings_weights(self.transformer.wte.weight)

#     def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, past=None, mask=None):
#         hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past, mask)
#         lm_logits = self.lm_head(hidden_states)
