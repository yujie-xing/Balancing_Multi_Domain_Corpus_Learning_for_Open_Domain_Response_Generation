...
class OpenAIGPTCLSHead(nn.Module):
    """ Classifier Head for the transformer """

    def __init__(self, config):
        super(OpenAIGPTCLSHead, self).__init__()
        self.n_embd = config.n_embd
        self.dropout = nn.Dropout(config.resid_pdrop)
    ...
    def forward(self, hidden_states):
        ...
        cls_h = self.dropout(hidden_states)
        cls_logits = self.linear(hidden_states)
...
class OpenAIGPTPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path, num_special_tokens=None, n_cls=0, state_dict=None, cache_dir=None, from_tf=False, *inputs, **kwargs
    ):
        """
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, mask=None):
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls_head = OpenAIGPTCLSHead(config)
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, mask=None, cls_id=None, cls_label=None):
        hidden_states = self.transformer(input_ids, position_ids, token_type_ids, mask)
        lm_logits = self.lm_head(hidden_states)
        # lm_logits = lm_logits[:,:,:self.vocab_size+1]
        loss = 0
        ...
        if cls_label is not None:
            ...
            # loss += (input_ids.size(-1)-cls_id)*loss_fct(cls_logits, cls_label)
            loss += loss_fct(cls_logits, cls_label)
        if loss:
            return loss
        try:
